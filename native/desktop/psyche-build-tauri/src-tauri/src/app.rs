...
pub fn run() {
    ...
    match acceptance::initialize(&context.config().identifier) {
        Ok(true) => {
            println!("acceptance profile prepared; GUI not launched");
            return;
        }
        Ok(false) => {}
        Err(error) => {
    ...
    let builder = if acceptance::active() {
    ...
    builder
        ...
        .invoke_handler(|invoke| {
            if !acceptance::command_allowed(invoke.message.command()) {
                invoke
                    .resolver
                    .reject("optional integration is disabled in the acceptance profile");
                return true;
            }
