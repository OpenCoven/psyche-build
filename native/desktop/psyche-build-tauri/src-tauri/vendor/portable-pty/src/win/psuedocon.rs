...
use crate::win_support::{check_win32_bool, WINDOWS_SPAWN_ATTRIBUTE_COUNT};
...
fn create_kill_on_close_job() -> Result<OwnedHandle, Error> {
    let raw_job = unsafe { CreateJobObjectW(ptr::null_mut(), ptr::null()) };
    ensure!(
        !raw_job.is_null(),
...
impl PsuedoCon {
    ...
    pub fn spawn_command(&self, cmd: CommandBuilder) -> anyhow::Result<WinChild> {
        ...
        let mut job_handles = [job.as_raw_handle() as HANDLE];
        let mut attrs = ProcThreadAttributeList::with_capacity(WINDOWS_SPAWN_ATTRIBUTE_COUNT)?;
