...
export class PopupManager {
    ...
    try {
      const popupHandle = await this.trackProjectActivity(async () => {
        ...
        if (tempData !== undefined) {
          tempFile = `/tmp/psyche-${scriptName.replace(".js", "")}-${Date.now()}.json`
  ...
  async launchNewPanePopup(projectPath?: string): Promise<string | null> {
    ...
    try {
      ...
      const projectName = effectivePath ? path.basename(effectivePath) : "psyche"
  ...
  > {
    ...
    try {
      ...
      const projectColorThemeSetting: SettingDefinition = {
        ...
        description: "Color for this project in the current psyche session. Auto picks an unused color; inherit follows the project's saved/default theme.",
        type: "select",
        scopeBehavior: "session",
        options: [
          { value: AUTO_SIDEBAR_PROJECT_COLOR_THEME_VALUE, label: "Auto" },
          { value: "", label: "Inherit Default Theme" },
          ...PSYCHE_THEME_NAMES.map((themeName) => ({
            value: themeName,
            label: getPsycheThemeLabel(themeName),
          })),
        ],
      ...
      try {
        // Use tmux client dimensions, not the psyche pane's stdout width.
