...
export const TOOLS: ToolDef[] = [
  {
    ...
    description: 'List the bounded pane and browser resources and approvals visible to one task.',
    inputSchema: { type: 'object', required: ['task_id'], properties: taskScopedReadProperties },
    handler: async (args) => {
  ...
  },
  {
    ...
    handler: async (args) => {
      ...
      return withControlClient(resolveProjectRoot(args), async (client) => (
        (await client.actionStatus(actionId, { taskId })) ?? { status: 'unknown', action_id: actionId }
      ));
    },
  },
  {
