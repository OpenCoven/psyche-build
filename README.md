...
| `psyche_control_action_status` | Read the latest task-scoped canonical receipt when available; redacted or missing bounded history reports `unknown` |
...
`psyche_list_panes` require `task_id` and fail clearly when it is omitted. The
compatibility mutation aliases require `task_id`, `lease_id`, and
