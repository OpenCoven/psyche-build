...
## Optional local session integration

Run this section only when a compatible local session provider is available.

1. Open the linked-worktree path itself as the project.
2. Confirm project open does not create an agent pane.
3. Launch a provider-backed terminal from Psyche Build and confirm it is scoped
   to the canonical project root.
4. Confirm exactly one active provider row appears for the Psyche-owned session.
5. Open that row twice and confirm the first action creates one visible
   attachment pane while the second focuses the existing attachment.
6. Confirm sessions from another project or another owning application do not
   appear in the Psyche Build rail.
7. Stop the provider and confirm Psyche Build shows one recoverable unavailable
   state while ordinary terminal, agent, worktree, file-browser, merge, and
   cleanup workflows remain usable.

Expected:

- Psyche Build exposes only sessions proven to belong to the current project.
- Opening or focusing an integration row never mutates provider ownership data.
- Provider loss remains isolated from Psyche Build's core workflow.
...
## Optional session bridge smoke

When a compatible local session provider is available for the same project:

- psyche can list and open project-scoped sessions through the bridge.
- launching a provider-backed session is scoped to the current project root.
- out-of-project `cwd` values are rejected before work starts.
- opening a session creates one visible attachment pane.
- event polling resumes from the provider cursor without duplicating events.
- stopped or missing providers surface a recoverable unavailable state instead
  of breaking normal psyche panes.
