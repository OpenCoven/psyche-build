...
| Diagnostics/support bundle | Current visible diagnostics only in `v0.0.1`; on `main` the support-bundle v1 schema (#243, PR #278) has since gained a CLI, bounded persistence and provenance/persistence/lifecycle/updater collectors (PRs #462, #467), with provider, graphics, receipt and terminal collectors and any UI still absent | Planned | `v0.0.1` does not claim a support bundle; #199's recovery harness is source-only, including the bounded pre-Git and mid-Git cleanup-owner interruption scenarios and the opt-in application-restart observation, not packaged operator acceptance |
