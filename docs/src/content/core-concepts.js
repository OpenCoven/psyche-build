...
export function render() {
  return `
    ...
    <ul>
      ...
      <li>Session state is stored in <code>.psyche/psyche.config.json</code></li>
    ...
    <ul>
      <li>A unique ID (e.g. <code>psyche-1</code>, <code>psyche-2</code>)</li>
    ...
    <ol>
      ...
      <li><strong>Monitor</strong> — psyche tracks agent status (working, analyzing, waiting, idle)</li>
    ...
    <p>psyche monitors each pane to detect the agent's state:</p>
    <table>
      <thead>
      ...
      <tbody>
        ...
        <tr><td><code>analyzing</code></td><td>psyche is analyzing recent terminal output</td></tr>
    ...
    <p>When a pane settles into a state that needs you, psyche marks it with an attention indicator in the sidebar and pane border. On macOS, psyche can also send a native notification for panes that are not currently focused.</p>
    <ul>
      ...
      <li>On non-macOS platforms, psyche still works normally without the native helper</li>
    </ul>

    <h3>Pane Visibility</h3>
    <p>psyche lets you temporarily remove panes from the active window without stopping them.</p>
    <ul>
    ...
    <ul>
      ...
      <li>Preview either file contents or git diff output without leaving psyche</li>
...
└── .psyche/worktrees/
    ...
    <p>When you merge a pane (press <kbd>m</kbd> to open the menu, then select merge), psyche performs a two-phase merge:</p>
    <ol>
    ...
    <div class="callout callout-info">
      ...
      If there are merge conflicts in step 2, psyche will abort the merge and let you know which files conflict. You can resolve them manually in the worktree and retry.
    </div>

    <h2>The Sidebar</h2>
    <p>The psyche TUI shows a sidebar with all active panes. Each pane displays:</p>
    <ul>
    ...
    <p>You can optionally use a sidebar layout mode (press <kbd>L</kbd>) where the psyche sidebar occupies a fixed 40-column pane on the left, with agent panes arranged in a grid to the right.</p>
