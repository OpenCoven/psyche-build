...
| `psyche_control_action_status` | `action_id`, `task_id` |
...
payloads. Agent task-scoped reads expose only resources named by persisted
capability-lease grants or pending lease requests, plus active approvals whose
lease id/revision still matches a visible task lease. Receipts are not
task-attributed durably and stay redacted from agent-scoped reads, so
`psyche_control_action_status` can legitimately return `unknown`.
