...
const debugRuntimeStressCapability = existsSync(debugRuntimeStressCapabilityPath)
  ? JSON.parse(readFileSync(debugRuntimeStressCapabilityPath, 'utf8'))
  : null;
...
function manifestAppCommands(name = 'APP_COMMANDS') {
  const match = tauriBuild.match(
    new RegExp(`const ${name}:\\s*&\\[&str\\]\\s*=\\s*&\\[(?<commands>[\\s\\S]*?)\\]\\s*;`),
  );
  expect(match?.groups?.commands).toBeTruthy();
...
describe('Tauri desktop tab shortcuts', () => {
  it('routes Command+T to terminal panes globally', () => {
    ...
    expect(mainJs).toMatch(
      /async function createTerminalPane\(\)\s*\{[\s\S]*var project = activeProject\(\);[\s\S]*if \(!project \|\| !project\.root \|\| project\.closing\)\s*\{[\s\S]*setStatus\("Open a project before starting a terminal", "warn"\);[\s\S]*return null;[\s\S]*\}[\s\S]*var worktree = selectedWorktree\(project\);[\s\S]*if \(!worktree \|\| !worktree\.path\)\s*\{[\s\S]*setStatus\("Select an available worktree before starting a terminal", "warn"\);[\s\S]*return null;[\s\S]*\}[\s\S]*await showTerminalView\(\)[\s\S]*return spawnShellThread\(project\);[\s\S]*\}/
    );
    expect(mainJs).toMatch(
  ...
  it('keeps an autogenerated allow and deny permission for every registered command', () => {
    for (const command of registeredAppCommands()) {
      const permission = readFileSync(
        join(
          repoRoot,
          'native/desktop/psyche-build-tauri/src-tauri/permissions/autogenerated',
          `${command}.toml`,
        ),
        'utf8',
      );
      const identifier = command.replaceAll('_', '-');
      expect(permission).toContain(`identifier = "allow-${identifier}"`);
      expect(permission).toContain(`commands.allow = ["${command}"]`);
      expect(permission).toContain(`identifier = "deny-${identifier}"`);
      expect(permission).toContain(`commands.deny = ["${command}"]`);
    }
  });
  ...
  it('keeps diagnostics restricted to the main app webview while browsers stay narrowly scoped', () => {
    ...
    expect(debugRuntimeStressCapability).toEqual({
      $schema: '../../gen/schemas/macOS-schema.json',
      identifier: 'main-runtime-render-stress',
      description: 'Allows only the main app webview to run the debug-only rendering stress harness',
      local: true,
      webviews: ['main'],
      permissions: [
    ...
    expect(debugRuntimeStressCapability.permissions).toHaveLength(2);
    expect(debugRuntimeStressCapability.windows).toBeUndefined();
    expect(JSON.stringify(debugRuntimeStressCapability)).not.toContain('psyche-browser-*');
  ...
  it('keeps the debug stress commands and permissions out of release manifests', () => {
    const releaseCommandsMatch = tauriBuild.match(
      /#\[cfg\(not\(debug_assertions\)\)\]\s*const APP_COMMANDS:\s*&\[&str\]\s*=\s*&\[(?<commands>[^\]]*)\];/,
    );
    expect(releaseCommandsMatch?.groups?.commands?.trim()).toBe('');
    expect(tauriBuild).toContain(
      'const RELEASE_PERMISSIONS_PATH_PATTERN: &str = "./permissions/autogenerated/[!d]*.toml";',
    );
    expect(tauriBuild).toMatch(
      /#\[cfg\(not\(debug_assertions\)\)\][\s\S]*permissions_path_pattern\(RELEASE_PERMISSIONS_PATH_PATTERN\)/,
    );
    expect(tauriBuild).toContain(
  ...
  it('validates and authorizes the fixed native minimize/restore cycle', () => {
    expect(tauriLib).toMatch(
      /async fn diagnostics_cycle_window\([\s\S]*webview: tauri::Webview[\s\S]*ensure_trusted_pty_caller\(webview\.label\(\)\)\?;[\s\S]*state\.ensure_stress_authorized\(\)\?;[\s\S]*let window = webview\.window\(\);[\s\S]*window\s*\.minimize\(\)[\s\S]*window\s*\.unminimize\(\)[\s\S]*window\s*\.set_focus\(\)/,
    );
  });
