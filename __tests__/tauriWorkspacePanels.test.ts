...
describe('Tauri workspace panels', () => {
  ...
  it('pins a repository-local Tauri 2 CLI for native builds', () => {
    expect(tauriPackage.scripts['build:web']).toContain(
      'esbuild web/control/control-entry.js --bundle --minify --format=iife --global-name=PsycheControl --outfile=web/control.bundle.js',
    );
    expect(tauriPackage.scripts['build:web']).toContain(
      'esbuild web/control/browser-automation-runtime-entry.js --bundle --minify --format=iife --outfile=web/control/browser-automation-runtime.js',
    );
