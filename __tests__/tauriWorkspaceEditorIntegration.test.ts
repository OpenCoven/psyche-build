...
describe('native CodeMirror workspace editor surface', () => {
  ...
  it('reports exact partial pane cleanup and retains the reconciled project', async () => {
    const project = { id: 'p1', root: '/project' };
    const first = { id: 'web-a', kind: 'web', projectId: project.id };
    const second = { id: 'web-b', kind: 'web', projectId: project.id };
    const state = { activeFileId: null, activeProjectId: project.id, activeThreadId: second.id,
      openFiles: [], projects: [project], threads: [first, second] };
    ...
    const removeProject = compileFunction<(id: string) => Promise<boolean>>(
      extractFunctionSource(mainJs, 'removeProject'), {
        ...
        findThread: (id: string) => state.threads.find((thread) => thread.id === id),
        closeBrowserPane: async (thread: typeof first) => {
        ...
        closeThread: () => false,
        replayBrowserResources: async (root: string) => { replayed.push(root); return true; },
    ...
    expect(state.projects).toEqual([project]);
    expect(state.threads).toEqual([second]);
    expect(replayed).toEqual(['/project']);
    expect(statuses).toEqual([[
      'project removal partially completed: 1 pane(s) closed; 1 failed (web-b); project retained', 'error',
    ]]);
  });

  it('completes project removal and reports provider-stop failure after pane cleanup', async () => {
    const project = { id: 'p1', root: '/project' };
    const thread = { id: 'web-a', kind: 'web', projectId: project.id };
    const state = { activeFileId: null, activeProjectId: 'other', activeThreadId: null,
      openFiles: [], projects: [project], threads: [thread] };
    const statuses: Array<[string, string]> = [];
    const removeProject = compileFunction<(id: string) => Promise<boolean>>(
      extractFunctionSource(mainJs, 'removeProject'), {
        findProject: () => project,
        ...
        guardDirtyFiles: async () => true,
        findThread: () => thread,
        closeBrowserPane: async () => { state.threads = []; return true; },
        closeThread: () => true,
        replayBrowserResources: async () => true,
        invoke: async (command: string) => {
          if (command === 'control_provider_stop') throw new Error('stop unavailable');
        },
        browserControlProviders: { '/project': Promise.resolve({}) },
        PsycheSessions: { invalidateCovenRequests: (value: unknown) => value },
        covenDiscovery: {},
        setStatus: (text: string, level: string) => statuses.push([text, level]),
        fileViewEl: { hidden: true }, terminalHost: { hidden: false },
        startCovenPolling: () => {}, refreshTabs: () => {},
        syncPaneMetricsVisibility: () => {}, syncProjectBrowser: () => {}, saveWorkspaceSoon: () => {},
      },
    );
    ...
    expect(statuses).toContainEqual([
      'project removed after control provider stop failed: Error: stop unavailable', 'warn',
    ]);
  });
