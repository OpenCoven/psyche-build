...
const originalDevWatch = process.env.PSYCHE_DEV_WATCH

afterEach(() => {
  if (originalDevWatch === undefined) {
    delete process.env.PSYCHE_DEV_WATCH
    return
  }

  process.env.PSYCHE_DEV_WATCH = originalDevWatch
})
...
describe("shouldUseQuietDevWatchExit", () => {
  ...
  it("returns false outside dev watch mode", () => {
    process.env.PSYCHE_DEV_WATCH = "false"
