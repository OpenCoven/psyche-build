...
it('rejects a non-existent leaf path reached through a symlink-escape', async () => {
  const root = await realpath(await mkdtemp(path.join(tmpdir(), 'psyche-scope-')));
  const outside = await mkdtemp(path.join(tmpdir(), 'psyche-outside-'));
  await mkdir(path.join(root, 'worktree'));
  await symlink(outside, path.join(root, 'escape'), 'dir');
  const scope = await ControlScope.create(root, {
  ...
  await expect(scope.requireContainedPath(path.join(root, 'escape', 'new-file.txt')))
    .rejects.toThrow('outside the canonical project');
  await expect(scope.requireContainedPath(path.join(root, 'worktree', 'brand-new.txt')))
    .resolves.toContain('brand-new.txt');
});
